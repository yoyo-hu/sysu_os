...
static void *thread_func(void *args) {
    ...
    thread_data1();
    ptr = (struct msg_struct *)pthread_getspecific(
        tls_key); /* get ptr from thread_data1() */
    perror("pthread_getspecific()");
    printf("ptr from thread_data1() in thread_func(): %p\n", ptr);
    for (int i = 1; i < 6; i++) {
        printf("tid = %ld  i = %2d   %s  %*.*s\n", gettid(), i,
               (ptr + i - 1)->stuno, 8, 8, (ptr + i - 1)->stuname);
    }

    thread_data2();
    ptr = (struct msg_struct *)pthread_getspecific(
        tls_key); /* get ptr from thread_data2() */
    perror("pthread_getspecific()");
...
void thread_data1(void) {
    ...
    pthread_setspecific(tls_key,
                        ptr); /* binding the tls_key to address of ptr */
    ...
    return;
    /* thread stack space is deallocated when thread_data1() returns and data
     * lost */
}

void thread_data2(void) {
    struct msg_struct *ptr;
    ptr = (struct msg_struct *)malloc(
        5 * sizeof(struct msg_struct)); /* in process heap */
    pthread_setspecific(tls_key,
                        ptr); /* binding the tls_key to address of ptr */
