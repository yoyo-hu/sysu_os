...
static void *thread_func1(void *args) {
    ...
    pthread_setspecific(tls_key,
                        ptr); /* binding its tls_key to address of ptr */
    for (int i = 0; i < 5; i++) {
        sprintf(ptr[i].pos, " ");
        sprintf(ptr[i].stuno, "        ");
...
void print_msg(void) {
    ...
    struct msg_struct *ptr = (struct msg_struct *)pthread_getspecific(
        tls_key); /* ptr decided by call thread */
