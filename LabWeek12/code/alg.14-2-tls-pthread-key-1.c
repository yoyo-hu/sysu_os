...
/* each thread associates the static global log_key with one local variable of
 * the thread */
/* the associated local variable behaves like a global variable by making use of
 * log_key */

void write_log(const char *msg) {
    FILE *fp_log;
    fp_log = (FILE *)pthread_getspecific(
        log_key); /* fp_log is shared in the same thread */
...
static void *thread_worker(void *args) {
    ...
    sprintf(fname, "log/thread-%d.log",
            ++thcnt); /* directory ./log must exist */
    ...
    pthread_setspecific(log_key,
                        fp_log); /* fp_log is associated with log_key */
