...
#define STACK_SIZE \
    1024 * 1024 /* 1Mib. question: what is the upperbound of STACK_SIZE */

static int child_func1(void *arg) {
    char *chdbuf = (char *)arg; /* type casting */
    printf("child_func1 read buf: %s\n", chdbuf);
    sleep(1);
    sprintf(chdbuf, "I am child_func1, my tid = %ld, pid = %d", gettid(),
            getpid());
...
static int child_func2(void *arg) {
    ...
    sleep(1);
    sprintf(chdbuf, "I am child_func2, my tid = %ld, pid = %d", gettid(),
            getpid());
...
int main(int argc, char **argv) {
    ...
    char *stack1 = malloc(
        STACK_SIZE *
        sizeof(char)); /* allocating from heap, safer than stack1[STACK_SIZE] */
    ...
    chdtid1 = clone(child_func1, stack1 + STACK_SIZE, flags | SIGCHLD,
                    buf); /* what happened if without SIGCHLD */
    if (chdtid1 == -1) {
        perror("clone1()");
        exit(1);
    }
    ...
    if (waitpid(-1, &status, 0) ==
        -1) { /* wait for any child existing, may leave some child defunct */
        perror("wait()");
    }
